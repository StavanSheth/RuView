...
/// State that is isolated per physical node to prevent signal interleaving.
struct NodeState {
    ...
    // ── Motion smoothing & adaptive baseline ──
    /// EMA-smoothed motion score.
    smoothed_motion: f64,
    /// Current classification state for hysteresis debounce.
    current_motion_level: String,
    /// Counter for consecutive frames agreeing on a new level.
    debounce_counter: u32,
    /// The candidate motion level being tracked for debounce.
    debounce_candidate: String,
    /// Adaptive baseline: EMA of motion score when room is "quiet".
    baseline_motion: f64,
...
impl Default for NodeState {
    fn default() -> Self {
        Self {
            ...
            vital_detector: VitalSignDetector::new(2.0), // Default 2Hz, will be overridden
            ...
            hr_buffer: VecDeque::with_capacity(VITAL_MEDIAN_WINDOW),
            br_buffer: VecDeque::with_capacity(VITAL_MEDIAN_WINDOW),
        }
    }
}
...
struct AppStateInner {
    ...
    /// Map of node states keyed by node_id.
    ...
    /// Most recent vital sign reading for the REST endpoint (aggregated/latest).
...
fn parse_esp32_frame(buf: &[u8]) -> Option<Esp32Frame> {
    ...
    // ADR-018: n_subcarriers is a u16 at bytes [6..7]
    let n_subcarriers = u16::from_le_bytes([buf[6], buf[7]]);
    // ADR-018: freq_mhz is a u32 at bytes [8..11]
    let freq_mhz = u32::from_le_bytes([buf[8], buf[9], buf[10], buf[11]]) as u16;
    // ADR-018: sequence is a u32 at bytes [12..15]
    let sequence = u32::from_le_bytes([buf[12], buf[13], buf[14], buf[15]]);
    // ADR-018: rssi at byte [16], noise_floor at byte [17]
    ...
    Some(Esp32Frame {
        ...
        node_id,
        n_antennas,
        n_subcarriers: n_subcarriers as u8,
        freq_mhz,
        sequence,
        rssi,
        noise_floor,
...
/// to the raw classification.  Mutates the smoothing state in `NodeState`.
fn smooth_and_classify(node: &mut NodeState, raw: &mut ClassificationInfo, raw_motion: f64) {
...
fn adaptive_override(node: &NodeState, features: &FeatureInfo, classification: &mut ClassificationInfo, adaptive_model: &Option<adaptive_classifier::AdaptiveModel>) {
    if let Some(ref model) = adaptive_model {
        let amps = node.frame_history.back()
            .map(|v| v.as_slice())
            .unwrap_or(&[]);
...
/// Mutates `node.smoothed_hr`, `node.smoothed_br`, etc.
/// Returns the smoothed VitalSigns to broadcast.
fn smooth_vitals(node: &mut NodeState, raw: &VitalSigns) -> VitalSigns {
    let raw_hr = raw.heart_rate_bpm.unwrap_or(0.0);
    let raw_br = raw.breathing_rate_bpm.unwrap_or(0.0);

    // -- Outlier rejection --
    let hr_ok = node.smoothed_hr < 1.0 || (raw_hr - node.smoothed_hr).abs() < HR_MAX_JUMP;
    let br_ok = node.smoothed_br < 1.0 || (raw_br - node.smoothed_br).abs() < BR_MAX_JUMP;

    // Push into buffer
    if hr_ok && raw_hr > 0.0 {
        node.hr_buffer.push_back(raw_hr);
        if node.hr_buffer.len() > VITAL_MEDIAN_WINDOW { node.hr_buffer.pop_front(); }
    }
    if br_ok && raw_br > 0.0 {
        node.br_buffer.push_back(raw_br);
        if node.br_buffer.len() > VITAL_MEDIAN_WINDOW { node.br_buffer.pop_front(); }
    }

    let trimmed_hr = trimmed_mean(&node.hr_buffer);
    let trimmed_br = trimmed_mean(&node.br_buffer);

    if trimmed_hr > 0.0 {
        if node.smoothed_hr < 1.0 {
            node.smoothed_hr = trimmed_hr;
        } else if (trimmed_hr - node.smoothed_hr).abs() > HR_DEAD_BAND {
            node.smoothed_hr = node.smoothed_hr * (1.0 - VITAL_EMA_ALPHA)
                              + trimmed_hr * VITAL_EMA_ALPHA;
        }
    }
    if trimmed_br > 0.0 {
        if node.smoothed_br < 1.0 {
            node.smoothed_br = trimmed_br;
        } else if (trimmed_br - node.smoothed_br).abs() > BR_DEAD_BAND {
            node.smoothed_br = node.smoothed_br * (1.0 - VITAL_EMA_ALPHA)
                              + trimmed_br * VITAL_EMA_ALPHA;
        }
    }

    // Smooth confidence
    node.smoothed_hr_conf = node.smoothed_hr_conf * 0.92 + raw.heartbeat_confidence * 0.08;
    node.smoothed_br_conf = node.smoothed_br_conf * 0.92 + raw.breathing_confidence * 0.08;

    VitalSigns {
        breathing_rate_bpm: if node.smoothed_br > 1.0 { Some(node.smoothed_br) } else { Some(0.0) }, // fallback to 0 instead of none for display
        heart_rate_bpm: if node.smoothed_hr > 1.0 { Some(node.smoothed_hr) } else { Some(0.0) },
        breathing_confidence: node.smoothed_br_conf,
        heartbeat_confidence: node.smoothed_hr_conf,
...
async fn windows_wifi_task(state: SharedState, tick_ms: u64) {
    ...
    loop {
        ...
        // Scope 1: Initial signal processing (needs node_state mutably)
        let (features, mut classification, breathing_rate_hz, sub_variances, _raw_motion) = {
            let node_state = s.nodes.entry(0).or_insert_with(NodeState::default);
            ...
            let (f, mut c, br_hz, sub_v, raw_m) = extract_features_from_frame(&frame, &node_state.frame_history, sample_rate_hz);
            smooth_and_classify(node_state, &mut c, raw_m);
            (f, c, br_hz, sub_v, raw_m)
        };
        
        // Apply adaptive override
        adaptive_override(&s.nodes[&0], &features, &mut classification, &s.adaptive_model);

        // Scope 2: Final processing (vitals and person count)
        let (vitals, est_persons, motion_score) = {
            let node_state = s.nodes.get_mut(&0).unwrap();
            
            node_state.rssi_history.push_back(first_rssi);
            if node_state.rssi_history.len() > 60 {
                node_state.rssi_history.pop_front();
            }

            let m_score = if classification.motion_level == "active" { 0.8 }
                else if classification.motion_level == "present_still" { 0.3 }
                else { 0.05 };

            let raw_v = node_state.vital_detector.process_frame(&frame.amplitudes, &frame.phases);
            let v = smooth_vitals(node_state, &raw_v);

            // Multi-person estimation.
            let raw_score = compute_person_score(&features);
            node_state.smoothed_person_score = node_state.smoothed_person_score * 0.85 + raw_score * 0.15;
            let ep = if classification.presence {
        ...
        // Re-extract enhanced pipeline fields
...
async fn windows_wifi_fallback_tick(state: &SharedState, seq: u32) {
    ...
    // Scope for initial signal processing (needs node_state mutably)
    let (features, mut classification, breathing_rate_hz, sub_variances, raw_motion) = {
        ...
        let sample_rate_hz = 2.0_f64;
        let (f, mut c, br_hz, sub_v, raw_m) = extract_features_from_frame(&frame, &node_state.frame_history, sample_rate_hz);
        smooth_and_classify(node_state, &mut c, raw_m);
        (f, c, br_hz, sub_v, raw_m)
    };

    // Apply adaptive override using individual field borrows (allowed by borrow checker)
    adaptive_override(&s.nodes[&0], &features, &mut classification, &s.adaptive_model);

    // Final processing (vitals and person count)
    let (vitals, est_persons, motion_score) = {
        let node_state = s.nodes.get_mut(&0).unwrap();
        node_state.rssi_history.push_back(rssi_dbm);
        if node_state.rssi_history.len() > 60 {
            node_state.rssi_history.pop_front();
        }
        let m_score = if classification.motion_level == "active" { 0.8 }
...
async fn vital_signs_endpoint(State(state): State<SharedState>) -> Json<serde_json::Value> {
    ...
    // For the status endpoint, we'll report the buffer status of node 0 (primary/wifi) if it exists.
    let (br_len, br_cap, hb_len, hb_cap) = if let Some(node) = s.nodes.get(&0).or_else(|| s.nodes.values().next()) {
        node.vital_detector.buffer_status()
    } else {
        (0, 0, 0, 0)
    };
    Json(serde_json::json!({
        ...
        match socket.recv_from(&mut buf).await {
            Ok((len, src)) => {
                ...
                if let Some(frame) = parse_esp32_frame(&buf[..len]) {
                    ...
                    // Scope 1: Initial signal processing (needs node_state mutably)
                    let (features, mut classification, breathing_rate_hz, sub_variances, _raw_motion) = {
                        let node_state = s.nodes.entry(node_id).or_insert_with(NodeState::default);
                        node_state.frame_history.push_back(frame.amplitudes.clone());
                        if node_state.frame_history.len() > FRAME_HISTORY_CAPACITY {
                            node_state.frame_history.pop_front();
                        }
                    ...
                    // Apply adaptive override
                    adaptive_override(&s.nodes[&node_id], &features, &mut classification, &s.adaptive_model);

                    // Scope 2: Final processing (vitals and person count)
                    let (vitals, est_persons, motion_score) = {
                        let node_state = s.nodes.get_mut(&node_id).unwrap();
                        node_state.rssi_history.push_back(features.mean_rssi);
                        if node_state.rssi_history.len() > 60 {
                            node_state.rssi_history.pop_front();
                        }
                        let m_score = if classification.motion_level == "active" { 0.8 }
